...
from timm.loss import JsdCrossEntropy, SoftTargetCrossEntropy, BinaryCrossEntropy, LabelSmoothingCrossEntropy, SymmetryLoss
...
group.add_argument('--wandb-project', default='', type=str,  help='WandDB project name')
...
def main():
    ...
    if utils.is_primary(args) and args.use_pair_sampling:
    ...
    if utils.is_primary(args) and args.log_wandb:
        if has_wandb:
            ...
            if len(args.wandb_runid) > 0:
                wandb.init(project=args.wandb_project, config=args, id=args.wandb_runid, resume='must')
            else:
